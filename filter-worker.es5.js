...
function GreyScale() {
    return function (rgba) {
        var avg = 0.299 * rgba.r + 0.587 * rgba.g + 0.114 * rgba.b;
        rgba.r = avg;
        rgba.g = avg;
        rgba.b = avg;
        return rgba;
...
var createStore = function createStore() {
    ...
    return {
        getBackupImgData: getBackupImgData,
        setBackupImgData: setBackupImgData
    };
...
var filterMap = {
    ...
    javana: [{
        ...
        use: Curves,
        params: ['b', [20, 0], [90, 120], [186, 144], [255, 230]]
    }, {
        use: Curves,
        params: ['r', [0, 0], [144, 90], [138, 120], [255, 255]]
    }, {
        use: Curves,
        params: ['g', [10, 0], [115, 105], [148, 100], [255, 248]]
    }, {
        use: Curves,
        params: ['rgb', [0, 0], [120, 100], [128, 140], [255, 255]]
    }],
    sepiana: [{
        use: SepiaCaman,
        params: [50]
...
function doFilter(imgData, filterName) {
    if (!!filterName && _instanceof(imgData, ImageData)) {
        ...
        var rgbaFuncList = filterMap[filterName].map(function (filter) {
            return filter.use.apply(filter, _toConsumableArray(filter.params));
        });
        ...
        var _loop = function _loop(i) {
            ...
            rgbaFuncList.forEach(function (rgbaFunc) {
                rgba = rgbaFunc(rgba);
            });
...
onmessage = function onmessage(e) {
    var _e$data = e.data,
        ...
        filterName = _e$data.filterName;

    if (_instanceof(imgData, ImageData) && !!filterName) {
        var result = doFilter(imgData, filterName);
        this.postMessage({
            result: result
        });
    } else {
